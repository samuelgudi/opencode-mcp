...
// Parse the optional tool filter up-front so we can emit a different
// instructions text when filtered mode is active. When the env var is
...
// When a filter is active, emit a short, accurate filtered-mode guide
// instead of the full tiered text. The full text references ~40 tool
// names and would be misleading with most of them filtered out.
const instructionsText = enabledTools
  ? buildFilteredInstructions(enabledTools, { defaultProvider, defaultModel })
  : fullInstructions;
...
// call so the filter observes every registration. `enabledTools` was
// parsed further up so it could also drive the instructions text.
